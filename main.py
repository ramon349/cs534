...
##my impleentation 
tester = Ridge() 
tester.fit(np.copy(diabetes.data),np.copy(diabetes.target),lmbd =0,coef_prior=None )

other =linear_model.Ridge(fit_intercept= True, alpha=0,normalize=False)
#X = (diabetes.data - np.mean(diabetes.data,axis=0) )/np.std(diabetes.data,axis=0) 
#other.fit(X,np.copy(diabetes.target))
other.fit(np.copy(diabetes.data) ,np.copy(diabetes.target))
